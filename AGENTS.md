...
- **Only the API parsing logic in `index.js`** (`fetchJobsPage`, `parseApiJobs`) is company-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
# E2E tests (real company API, SOLR conditional)
npm run test:e2e
...
- `tests/validate-gaminvest-jobs.js` — CI fast validator (HEAD only); thin wrapper over src/job-validator.js + solr.js
