...
This scraper extracts job listings from GAMINVEST careers page (Romania only) and imports them to peviitor.ro.

Target: https://www.gaminvest.ro/cariere.html
...
1. **Start with brand** - We know the brand (e.g., "GAMINVEST")
...
8. **Scrape new jobs** - Extract jobs from GAMINVEST careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts GAMINVEST Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from GAMINVEST careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape GAMINVEST API (jobs for Romania)
    │
    ▼
...
| `tests/validate-gaminvest-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real GAMINVEST API |
