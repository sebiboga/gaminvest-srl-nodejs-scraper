...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-gaminvest-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for GAMINVEST used in unit tests |
| `tests/validate-gaminvest-jobs.js` | **GAMINVEST-specific fast validator (used by CI).** HEAD requests only, hardcoded GAMINVEST CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real GAMINVEST API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for GAMINVEST Careers |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 21913994. Use only when you need to wipe GAMINVEST jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape GAMINVEST → transform → upsert SOLR → generate docs/jobs.md